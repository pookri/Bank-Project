...
t1.transaction_account_number, 
t1.transaction_type,
--tt1.transactiontype_name,
t1.transaction_time, 
t1.transaction_amount, 
b1.changed_balance
FROM transaction t1
INNER JOIN balance_logs b1 ON t1.transaction_account_number = b1.transaction_account_number
INNER JOIN transaction_type tt1 ON tt1.transaction_type = t1.transaction_type
where t1.transaction_account_number=163951695;
...
SELECT ac.account_number,ac.acc_type FROM account ac,customer c, customer_account ca WHERE ac.account_number=ca.account_number AND ca.cssn=c.cssn;
...
SELECT employee_ssn,e_firstname,e_lastname FROM employee e where e.branch_id='AXS2'
...
SELECT transaction_account_number,transaction_type,transaction_amount,transaction_time,transaction_date,transaction_id FROM transaction where transaction_account_number=8012965589 ORDER BY transaction_time DESC;
...
SELECT a.account_number,a.branch_id,a.acc_type,a.create_date,ca.recent_access_date,c.c_firstname,c.c_lastname FROM account a,customer_account ca, customer c where ca.account_number=a.account_number AND c.cssn=ca.cssn;
...
Select * from Transaction_balance where TRANSACTION_ACCOUNT_NUMBER=8012965589
